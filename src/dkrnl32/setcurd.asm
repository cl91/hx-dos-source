...
if 0
		...
		sub 	bl,'a'-1
		push	ebx
		mov 	ax,4409h		   ;kann nicht pruefen, ob von floppy
		int 	21h 			   ;gelesen werden kann
		pop 	edx
		jc		chdir_er
endif
		mov 	al,[edx]			;falls wir nur "X:" angegeben haben
		or		al,20h				;geschieht kein Zugriff allein durch
		sub 	al,'a'-1			;AH=0E. Deshalb machen wir getcwd()
		mov 	dl,al				;A:=1,B:=2
		cmp 	byte ptr [edx+2],0	;pfad mitgegeben?
		jnz 	@F
		mov 	ah,47h				;get current dir (LFN?)
		int 	21h
		jc 		chdir_er
@@:
